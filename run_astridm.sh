...
#        fi
	...
	then
		method=astriddef
		sed "s/_[0-9]*_[0-9]*//g" ${i}_basaltrifurcation > ${i}_basaltrifurcation_sp
		/usr/bin/time -a -o ${file}.${method}.time $ASTRID -i ${i}_basaltrifurcation_sp -c ${file}.${method}.cache -o ${file}.${method} 1> ${file}.${method}.out 2>${file}.${method}.err
	fi
done
